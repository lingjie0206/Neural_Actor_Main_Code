...
class NSVFModel(NeRFModel):
    ...
    def intersecting(self, ray_start, ray_dir, encoder_states, **kwargs):
        ...
        if getattr(self.args, "fixed_num_samples", 0) > 0:
            intersection_outputs['steps'] = intersection_outputs['min_depth'].new_ones(
                *intersection_outputs['min_depth'].size()[:-1]) * self.args.fixed_num_samples
        else:
    ...
    def postprocessing(self, ray_start, ray_dir, all_results, hits, sizes):
        ...
        if 'colors' in all_results:
            ...
            if self.reader.curr_expos is not None:
                scale = torch.abs(self.reader.curr_expos[:,:,None,0].expand(S, V, P)) + 0.5
                shift = self.reader.curr_expos[:,:,None,1].expand(S, V, P)
                masks = hits.reshape(S, V, P)
                scale, shift = scale.masked_fill(~masks, 1.0), shift.masked_fill(~masks, 0.0)
                all_results['colors'] = (all_results['colors'] - shift.unsqueeze(-1)) / scale.unsqueeze(-1)
...
@register_model_architecture("sdf_nsvf", "sdf_nsvf_64")
