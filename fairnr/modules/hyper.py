...
class MoEFCLayer(nn.Module):
    
    def __init__(self, in_dim, out_dim, num_experts=24, nonlinear=True, switch=False):
        super().__init__()
        self.num_experts = num_experts
        self.weights = nn.Parameter(
            torch.zeros(num_experts, out_dim, in_dim).normal_(0, 0.01))
        self.biases = nn.Parameter(
            torch.zeros(num_experts, 1, out_dim))
        self.nonlinear = nonlinear
    
    def forward(self, inputs):
        """assuming idx is already sorted."""
        x, sizes = inputs
        ...
        for i, s in enumerate(sizes):
            ...
            if s > 0:
                xs += [torch.einsum('nd,cd->nc', x[start: end], weights) + biases]
            start = end
        x = torch.cat(xs, 0)
        if self.nonlinear:
            x = torch.relu(x)
